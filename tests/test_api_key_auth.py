...
gates them behind a shared secret (QUANTHORIZON_API_KEY) when configured, while staying a no-op
...
def test_require_api_key_is_noop_when_unconfigured(monkeypatch):
    monkeypatch.setattr(appmod, "QUANTHORIZON_API_KEY", "")
...
def test_require_api_key_rejects_missing_header_when_configured(monkeypatch):
    monkeypatch.setattr(appmod, "QUANTHORIZON_API_KEY", "s3cr3t")
